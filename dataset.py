...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        trans1 = nn.Sequential(
            T.RandomResizedCrop((224, 224), scale=(0.2, 0.75)),
            RandomApply(
        ...
        trans2 = nn.Sequential(
            T.RandomResizedCrop((224, 224), scale=(0.95, 1)),
            RandomApply(
                T.ColorJitter(0.8, 0.8, 0.8, 0.2),
                p = 0.3
            ),
        ...
        i1 = tensor_to_np(img1)
        i2 = tensor_to_np(img2)
        ...
        canav1 = np.empty((224, 224, 3))
        canav2 = np.empty((224, 224, 3))
                

        canav3 = np.empty((224, 224, 3))
        canav4 = np.empty((224, 224, 3))


        for i in range(24):
            pos_a1, pos_b1 = ((x[i] // 7) * 32, (x[i] % 7) * 32)
            patch_rgb1 = i1[int(pos_a1) : int(pos_a1) + 32, int(pos_b1) : int(pos_b1) + 32, 0:]

            pos_a2, pos_b2 = ((y[i] // 7) * 32, (y[i] % 7) * 32)
            patch_rgb2 = i2[int(pos_a2) : int(pos_a2) + 32, int(pos_b2) : int(pos_b2) + 32, 0:]

            p1, p2 = ((i // 7) * 32, (i % 7) * 32)
            canav1[int(p1) : int(p1) + 32, int(p2) : int(p2) + 32, 0:] = patch_rgb1
            canav2[int(p1) : int(p1) + 32, int(p2) : int(p2) + 32, 0:] = patch_rgb2
            ...
            # p1, p2 = ((i // 7) * 32, (i % 7) * 32)
            # canav3[int(p1) : int(p1) + 32, int(p2) : int(p2) + 32, 0:] = patch_rgb1
            # canav4[int(p1) : int(p1) + 32, int(p2) : int(p2) + 32, 0:] = patch_rgb2

        canav1 = canav1.astype(np.uint8)
        canav2 = canav2.astype(np.uint8)
        canav3 = canav3.astype(np.uint8)
        canav4 = canav4.astype(np.uint8)
...
if __name__ == '__main__':
    ...
    Myset = MyDataset("H:\\硕士资料\\IPM-paper\\img", transform = trans)
    Myset.__getitem__(339) # 248 72
